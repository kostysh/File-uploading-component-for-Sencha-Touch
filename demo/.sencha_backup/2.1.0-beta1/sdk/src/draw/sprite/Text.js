...
Ext.define("Ext.draw.sprite.Text", {
    ...
    constructor: function (config) {
        this.superclass.constructor.prototype.constructor.call(this, config);
        this.setAttributes({
            fillStyle: "#000"
        });
    },
    ...
    getBBox: function (isWithoutTransform) {
        var me = this,
            attr = me.attr,
            bbox = attr.bbox,
            x, y, width, height, size, font, text, baseline, alignment;
        ...
        if (bbox.plain) {
            ...
            x = bbox.plain.x;
            y = bbox.plain.y;
            width = bbox.plain.width;
            height = bbox.plain.height;
        } else {
            x = attr.x;
            y = attr.y;
            font = attr.font;
        ...
        if (!isWithoutTransform) {
            ...
            var matrix = attr.matrix,
                matArray = matrix.elements,
                transWidth = Math.abs(width * matArray[0]) + Math.abs(height * matArray[2]),
                transHeight = Math.abs(width * matArray[1]) + Math.abs(height * matArray[3]),
                cx = matrix.x(x + width * 0.5, y + height * 0.5),
                cy = matrix.y(x + width * 0.5, y + height * 0.5);

            return bbox.transform = {
                x: cx - transWidth * 0.5,
                y: cy - transHeight * 0.5,
                width: transWidth,
                height: transHeight
            };
        } else {
    ...
    render: function (surface, ctx) {
        ...
        if ((surface instanceof Ext.draw.engine.Svg) || this.divBased) {
            ...
            mat.prepend(1, 0, 0, 1, surface.getRegion()[0], surface.getRegion()[1]);
